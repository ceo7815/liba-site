...
const values = [
  ...
  { icon: <Shield className="w-7 h-7" />, title: "תמונה רחבה", desc: "ביטוח + פנסיה + פיננסים + פרישה + מיצוי זכויות — הכול תחת מעטפת אחת." },
...
const differentiators = [
  ...
  "מביאים ניסיון מהשטח בתחומי ביטוח, פנסיה, פיננסים ומיצוי זכויות",
  "לא רק מסבירים: ממלאים טפסים יחד ומלווים עד תוצאה בפועל",
  "זמינים כשצריך — באמת",
];

const whatWeDo = [
  { icon: <Shield className="w-8 h-8" />, title: "ביטוח למשפחה", desc: "בריאות, חיים, משכנתא, מחלות קשות ונסיעות לחו״ל — כיסוי מותאם למצב שלכם." },
  { icon: <Heart className="w-8 h-8" />, title: "פנסיה ופרישה", desc: "תכנון פרישה, קרנות השתלמות ובדיקת מצב פנסיוני." },
  { icon: <Users className="w-8 h-8" />, title: "פיננסי וזכויות", desc: "תכנון פיננסי, הלוואות, משיכות, מס, תביעות ועיקולים — ליווי עד שיש תוצאה בפועל." },
];
...
const faqs = [
  ...
  { question: "האם אתם מטפלים גם בתביעות?", answer: "כן. במסגרת ליבה מלווים תביעות מול חברות הביטוח וביטוח לאומי, משיכות, פטורי מס פנסיוניים והסרת עיקולים — אחרי בדיקה, בלי הבטחת סכום מראש." },
];
...
const AboutPage = () => {
  return (
    <main id="main-content" className="pt-20">
        ...
        <div className="container mx-auto max-w-4xl relative">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-lg md:text-xl text-primary-foreground/80 leading-relaxed max-w-3xl">
              ליבה ביטוח ופנסיוני היא חברת ליווי ביטוחי-פיננסי שמטרתה אחת: לגרום לתיק שלכם להיות ברור, מותאם ומבוקר — וגם ללוות עד תוצאה כשמדובר בזכויות, משיכות ומס.
            </p>
            <p className="text-primary-foreground/70 mt-4 text-base">
              הגישה שלנו: קודם להבין → אחר כך להמליץ → ואז ליישם בצורה נקייה ושקופה. לא שולחים לטפסים לבד.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      {/* Approach — rights and pension experience as part of Liba */}
      <section className="section-padding bg-background">
        <div className="container mx-auto max-w-3xl">
          <h2 className="font-heading text-3xl font-bold mb-6 text-center">לא רק ליווי — גם הישורת האחרונה</h2>
          <div className="space-y-5 text-muted-foreground leading-relaxed text-lg">
            <p>
              בליבה נפגשו אנשי מקצוע עם לב גדול וראש חד, מעולם הפנסיה והביטוח — מקום שבו כל מילה קטנה יכולה להשפיע על חיים שלמים.
            </p>
            <p>
              חלק מהצוות עבד שנים מהצד השני: ראינו איך פועלות חברות הביטוח, הגופים הממשלתיים ומס הכנסה, ואיפה הלקוח נופל בין הכיסאות.
            </p>
            <p>
              לכן בליבה לא מסתפקים בהסבר. ממלאים טפסים יחד, מלווים מול הרשויות, ולא עוזבים עד שיש תוצאה בפועל — כסף בחשבון או זכות שאושרה.
            </p>
            <p>
              מיצוי זכויות, משיכות כספים ופטורי מס פנסיוניים ניתנים במסגרת ליבה ביטוח ופנסיוני. המידע באתר הוא כללי, ואינו ייעוץ מס או ייעוץ פנסיוני אישי בלי בדיקה.
            </p>
          </div>
        </div>
      </section>
