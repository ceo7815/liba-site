...
const LPGovBenefitsThankYouPage = () => {
  useClarityPageTags({ pageType: "landing-page", lpCampaign: "government-benefits", funnelStep: "thank-you" });

  useEffect(() => { trackLead(); setClarityTag("lead_campaign", "government-benefits"); trackClarityEvent("lead_submitted"); }, []);
  return (
  <main className="min-h-screen bg-background" dir="rtl">
    <SEOHead
      title="תודה! הבדיקה בדרך אליך | ליבה ביטוח ופנסיוני"
      description="קיבלנו את הפרטים שלך. ניצור איתך קשר בהקדם עם תוצאות בדיקת הזכאות להטבות ביטוח משכנתא."
      canonical="/lp/government-benefits/thank-you"
      noindex
    />
    ...
    <div className="container mx-auto px-4 py-16 md:py-24 max-w-lg text-center">
      ...
      >
        ...
        <p className="text-muted-foreground text-lg mb-8">
          הבדיקה שלך בתהליך — ניצור איתך קשר בהקדם עם התוצאות.
        </p>
      </motion.div>
      ...
      >
        ...
        <div className="space-y-4">
          {[
            ...
            { icon: <Clock className="w-6 h-6 text-brand-gold" />, text: "ניצור איתך קשר תוך 24 שעות עם תוצאות הבדיקה" },
            { icon: <CheckCircle className="w-6 h-6 text-accent" />, text: "אם קיימת הטבה — נציג לך את ההצעה ותחליט" },
          ].map((item) => (
...
export default LPGovBenefitsThankYouPage;
