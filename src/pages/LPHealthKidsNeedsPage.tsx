...
const LPHealthKidsNeedsPage = () => {
  useClarityPageTags({ pageType: "landing-page", lpCampaign: "health-kids", funnelStep: "needs" });

  const navigate = useNavigate();

  const select = (need: string) => {
    try {
      sessionStorage.setItem("hk_need", need);
    } catch {
      /* ignore */
    }
  ...
  return (
    <main className="min-h-screen bg-background flex flex-col" dir="rtl">
      <SEOHead
        ...
        canonical="/lp/health-kids/needs"
        noindex
      />
...
export default LPHealthKidsNeedsPage;
