...
const WEBHOOK_URL = "https://hook.eu2.make.com/fksoilsp4muu46d71rw5113j8fhcc6f7";
...
const LPHealthKidsCheckPage = () => {
  useClarityPageTags({ pageType: "landing-page", lpCampaign: "health-kids", funnelStep: "check" });
  ...
  // Prefill need from sessionStorage
  useEffect(() => {
    try {
      const savedNeed = sessionStorage.getItem("hk_need");
      if (savedNeed) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      meta_source_page: "lp-health-kids",
    ...
    try {
      ...
      markLeadPending();
      setTimeout(() => navigate("/lp/health-kids/thank-you"), 500);
    } catch (error) {
  ...
  return (
    <main className="min-h-screen bg-background" dir="rtl">
      <SEOHead
        ...
        canonical="/lp/health-kids/check"
        noindex
      />
      ...
      <section className="relative py-14 md:py-16 overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-4 text-center">
          <img src={logo} alt={`לוגו ${siteConfig.name}`} className="h-12 md:h-14 mx-auto mb-5" />
        ...
        >
          ...
          <div>
            <input
              ...
              aria-describedby={phoneError ? "phone-error-hk" : undefined}
              className={inputClass}
            />
            {phoneError && (
              <p id="phone-error-hk" className="text-destructive text-xs mt-1 px-1">{phoneError}</p>
            )}
          </div>
...
export default LPHealthKidsCheckPage;
